# MCP not applicable: 9 GTM tools, counted

> 9 of 1,032 GTM tools in this directory are mcp not applicable. An MCP server is not a meaningful question for this entry.
...
---
[Directory](../index.md) /
[By MCP status](index.md) / MCP not applicable

**n-a**

## MCP not applicable

An MCP server is not a meaningful question for this entry.

9 of 1,032 entries. Ordered by the published rule: official MCP first, then community, then unknown, then n/a, then none-found; within each band gate order is free, paid, enterprise-leaning, enterprise-only, unknown; then alphabetical by name. Computed, never curated, never purchasable.
