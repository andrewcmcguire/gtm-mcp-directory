...
2. The product is checked against all 1,032 existing entries by normalised name, because 16 deliberate cross listings already exist.
...
Open an issue on the same repo naming the entry and the field. A correction is the most valuable thing anyone can send. The whole reason this is free and public is that other operators correct it faster than one person can re-check 1,032 entries.
