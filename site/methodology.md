...
> The five laws an entry survives, the two honesty tiers (1 bench tested of 422), the counting authority, and every thin spot named rather than padded.
...
The list is not the moat. Anyone can copy 422 rows. What is hard to copy is that every answer carries its honesty tier and the date it was measured, and that the awkward numbers are on the page instead of in a drawer.

**The two tiers**

**RESEARCHED.** Facts from public sources with URLs. No usage claims. Nobody has run this tool. All 421 entries in this build are RESEARCHED.
...
2. Unknown is a legal answer. 119 entries carry an unknown access gate and they are published as unknown rather than guessed into a bucket.
...
159 entries are none-found. That is a statement about the search, made on the date in the entry, and it is not a promise that no server exists. A vendor who shipped one the week after the check is recorded as none-found until the next pass, which is exactly why the weekly diff exists and why every entry ships its last_checked date.

**The counting authority**

tools_recount.py is the counter, not this site and not the build script. The build reconciles against it file by file and fails rather than publish a drifted number. This build: 422 against 422, 0 failures, 0 parser warnings. The site generator re-checks the same numbers before it writes a single file.

Data baked 2026-09-12 by build_directory.py (phase 1). Network calls made during the build: 0. Content sha256 24a2f58fd2e14d8e9066a041...

**The duplicates, and why two counts exist**

422 entries, 406 unique products. The difference is 16 products that are deliberately listed in two category files because a reader browsing either one should find them. The canonical home for each is declared in INDEX.md and not chosen by the parser. Category and status views count all 422 entries, because that is what the source files hold. Tool pages count 406, because that is how many products there are.
...
**119 entries have an unknown access gate** and **347 have no documentation URL.** Both are legal and both are published as blank. Every one of them is visible on its own tool page.
...
- docs_digest: empty on 422 of 422 entries
- docs_last_crawled: empty on 422 of 422 entries
- github_archived: empty on 422 of 422 entries
- github_fetched_on: empty on 422 of 422 entries
- github_last_commit: empty on 422 of 422 entries
- github_stars: empty on 422 of 422 entries
- github_url: empty on 422 of 422 entries
- submission: empty on 422 of 422 entries
...
jobs[] used to be on the list above. It is not any more. As of 2026-08-25 the vocabulary is closed at 56 jobs in 10 families, and 849 tags are assigned across 271 of 422 entries. 151 entries carry no tag at all, each one for a recorded reason that is printed on its own tool page.
