...
An MCP server does what your credential allows, not what the tool description says. A CRM key scoped to full access does not become read only because the agent was only asked to read. Scope the credential at the vendor, not in the prompt. Of the 437 servers counted here, [the OAuth ones](../lists/auth-oauth.md) are the better shape for this: the token is scoped and revocable from the vendor side without touching the agent's config.
