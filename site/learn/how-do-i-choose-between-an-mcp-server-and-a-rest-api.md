...
| Available for | 303 of 604 entries here | Most of the rest, if they document one |

## The honest default

Start with the server if there is one. The whole point of a protocol is that you stop writing the same integration twice, and 200 vendors here have already done the work for you. Move to the API when you hit a specific wall, and you will know exactly which wall it was.
...
- **The server is community built and you cannot carry the risk.** 103 entries here are in that position.

- **There is no server.** 277 entries, and 449 of 604 entries have no documentation URL recorded either, which is its own kind of answer.
