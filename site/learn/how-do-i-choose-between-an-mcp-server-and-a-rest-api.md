...
| Available for | 741 of 1,252 entries here | Most of the rest, if they document one |

## The honest default

Start with the server if there is one. The whole point of a protocol is that you stop writing the same integration twice, and 201 vendors here have already done the work for you. Move to the API when you hit a specific wall, and you will know exactly which wall it was.
...
- **There is no server.** 487 entries, and 727 of 1,252 entries have no documentation URL recorded either, which is its own kind of answer.

## The thing that decides it more often than either

The access gate. 79 entries need a contract before you get any credential at all, and at that point the protocol question is academic. Check [the gate](../gates/index.md) before you design either integration.
