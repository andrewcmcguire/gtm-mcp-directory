...
> Of the 207 GTM tools with an MCP server, 121 document an OAuth flow and 109 document an API key. Why the difference matters, and the full breakdown. Counted 2026-09-07.
...
Of the 207 GTM tools with an MCP server in this directory, 121 document an OAuth or browser sign in flow and 109 document an API key or token. Some document both, usually OAuth for the hosted server and a key for the self hosted one.
...
|---|---|---|
| [OAuth](../lists/auth-oauth.md) | 63 | The server takes the user through a browser sign in and holds a scoped token. Nothing is pasted into a config... |
| [API key](../lists/auth-api-key.md) | 51 | The server authenticates with a key or token the operator generates and pastes in. Simple to wire, and the... |
| [OAuth or an API key](../lists/auth-either.md) | 58 | Both paths are documented. Usually OAuth for a hosted server and a key for the self hosted or legacy endpoint. |
...
Counted 2026-09-07 across the 207 entries with a server. The bucket is a keyword match over the mcp_auth field, run at build time and disclosed as such; the verbatim field is printed beside every row on [the auth pages](../lists/auth-types.md) so you can check the parse yourself. 313 of 318 entries record an auth value at all.
