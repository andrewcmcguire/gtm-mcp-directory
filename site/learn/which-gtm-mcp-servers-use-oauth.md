...
> Of the 165 GTM tools with an MCP server, 91 document an OAuth flow and 89 document an API key. Why the difference matters, and the full breakdown. Counted 2026-08-25.
...
Of the 165 GTM tools with an MCP server in this directory, 91 document an OAuth or browser sign in flow and 89 document an API key or token. Some document both, usually OAuth for the hosted server and a key for the self hosted one.
...
| [OAuth or an API key](../lists/auth-either.md) | 45 | Both paths are documented. Usually OAuth for a hosted server and a key for the self hosted or legacy endpoint. |
...
Counted 2026-08-25 across the 165 entries with a server. The bucket is a keyword match over the mcp_auth field, run at build time and disclosed as such; the verbatim field is printed beside every row on [the auth pages](../lists/auth-types.md) so you can check the parse yourself. 288 of 293 entries record an auth value at all.
