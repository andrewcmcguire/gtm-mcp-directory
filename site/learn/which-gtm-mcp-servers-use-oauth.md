...
> Of the 281 GTM tools with an MCP server, 137 document an OAuth flow and 121 document an API key. Why the difference matters, and the full breakdown. Counted 2026-09-12.
...
Of the 281 GTM tools with an MCP server in this directory, 137 document an OAuth or browser sign in flow and 121 document an API key or token. Some document both, usually OAuth for the hosted server and a key for the self hosted one.
...
| [Auth not recorded](../lists/auth-unrecorded.md) | 84 | The mcp_auth field on the entry is blank, or says unknown. Published as blank rather than guessed. |

Counted 2026-09-12 across the 281 entries with a server. The bucket is a keyword match over the mcp_auth field, run at build time and disclosed as such; the verbatim field is printed beside every row on [the auth pages](../lists/auth-types.md) so you can check the parse yourself. 331 of 559 entries record an auth value at all.
