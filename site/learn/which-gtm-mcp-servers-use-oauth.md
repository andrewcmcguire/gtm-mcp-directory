...
> Of the 741 GTM tools with an MCP server, 137 document an OAuth flow and 122 document an API key. Why the difference matters, and the full breakdown. Counted 2026-09-15.
...
Counted 2026-09-15 across the 741 entries with a server. The bucket is a keyword match over the mcp_auth field, run at build time and disclosed as such; the verbatim field is printed beside every row on [the auth pages](../lists/auth-types.md) so you can check the parse yourself. 332 of 1,252 entries record an auth value at all.
