...
> Of the 741 GTM tools with an MCP server, 137 document an OAuth flow and 122 document an API key. Why the difference matters, and the full breakdown. Counted 2026-09-14.
...
Of the 741 GTM tools with an MCP server in this directory, 137 document an OAuth or browser sign in flow and 122 document an API key or token. Some document both, usually OAuth for the hosted server and a key for the self hosted one.
...
|---|---|---|
| [OAuth](../lists/auth-oauth.md) | 69 | The server takes the user through a browser sign in and holds a scoped token. Nothing is pasted into a config... |
| [API key](../lists/auth-api-key.md) | 54 | The server authenticates with a key or token the operator generates and pastes in. Simple to wire, and the... |
...
| [Auth not recorded](../lists/auth-unrecorded.md) | 543 | The mcp_auth field on the entry is blank, or says unknown. Published as blank rather than guessed. |

Counted 2026-09-14 across the 741 entries with a server. The bucket is a keyword match over the mcp_auth field, run at build time and disclosed as such; the verbatim field is printed beside every row on [the auth pages](../lists/auth-types.md) so you can check the parse yourself. 332 of 1,252 entries record an auth value at all.
