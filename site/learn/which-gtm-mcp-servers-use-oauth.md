...
> Of the 167 GTM tools with an MCP server, 95 document an OAuth flow and 91 document an API key. Why the difference matters, and the full breakdown. Counted 2026-09-02.
...
Counted 2026-09-02 across the 167 entries with a server. The bucket is a keyword match over the mcp_auth field, run at build time and disclosed as such; the verbatim field is printed beside every row on [the auth pages](../lists/auth-types.md) so you can check the parse yourself. 288 of 293 entries record an auth value at all.
