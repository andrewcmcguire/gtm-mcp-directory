...
> Of the 165 GTM tools with an MCP server, 92 document an OAuth flow and 90 document an API key. Why the difference matters, and the full breakdown. Counted 2026-08-28.
...
Of the 165 GTM tools with an MCP server in this directory, 92 document an OAuth or browser sign in flow and 90 document an API key or token. Some document both, usually OAuth for the hosted server and a key for the self hosted one.
...
|---|---|---|
| [OAuth](../lists/auth-oauth.md) | 46 | The server takes the user through a browser sign in and holds a scoped token. Nothing is pasted into a config... |
...
| [OAuth or an API key](../lists/auth-either.md) | 46 | Both paths are documented. Usually OAuth for a hosted server and a key for the self hosted or legacy endpoint. |
...
| [Auth not recorded](../lists/auth-unrecorded.md) | 25 | The mcp_auth field on the entry is blank, or says unknown. Published as blank rather than guessed. |

Counted 2026-08-28 across the 165 entries with a server. The bucket is a keyword match over the mcp_auth field, run at build time and disclosed as such; the verbatim field is printed beside every row on [the auth pages](../lists/auth-types.md) so you can check the parse yourself. 288 of 293 entries record an auth value at all.
