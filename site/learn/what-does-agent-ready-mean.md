...
- **Is there a server at all.** 200 of 739 entries have an official one, 159 have a community one, 356 had none found on the date checked, and 15 could not be settled.
...
- **Can you get in.** 210 of 739 entries are free to start or paid self serve, and 79 need a contract. Cross that with the server column and 168 entries pass both tests. An official MCP server behind a procurement cycle is not agent ready for most of the people reading this.
...
The reverse trap is real too. A tool with no MCP server and a clean, documented REST API is often more usable to an agent than a tool with a thin server and no docs. 510 of 739 entries have no documentation URL recorded at all, which is its own signal.
