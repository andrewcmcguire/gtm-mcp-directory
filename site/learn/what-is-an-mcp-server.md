...
> An MCP server is a small program that exposes one system's capabilities to an AI agent in a standard shape. 200 of 514 GTM tools ship one officially. Counted 2026-09-12.
...
This directory checked 514 go to market tools and found **200 with an official server**, the vendor's own, plus 64 where somebody outside the vendor built one. That is 51% of the entries reachable by an agent through MCP at all. 226 had no server found on the date they were checked, and 15 could not be settled either way.
...
- **How it authenticates.** 69 of the 264 servers in this directory use OAuth, which keeps a revocable scoped token on the vendor side. Others take an API key you paste into a config file. [The full auth breakdown is here](../lists/auth-types.md).
