...
> 21 of the 70 data enrichment tools in this directory are free to start, and 18 of those also have an MCP server. The list, with what each one does. Counted 2026-09-12.
...
21 of the 70 data enrichment tools counted here are free to start, meaning a solo operator can get API access without paying and without talking to anyone. 18 of those also ship an MCP server, so an agent can call them without any glue code.
...
Data & Enrichment is the most agent reachable category in the whole directory: 42 official servers and 2 community across 70 entries, with only 26 where none was found. These vendors were selling an API before MCP existed, so exposing it through one more protocol was a small step.
