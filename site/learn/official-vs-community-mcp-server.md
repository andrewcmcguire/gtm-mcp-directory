...
> Official means the vendor ships and maintains the server. Community means somebody else does. 200 against 159 across 739 GTM tools.
...
200 entries are official. 159 are community. 356 had no server found at all, and 15 could not be settled either way and are published as unknown rather than guessed into a bucket.

Community servers cluster. Video Prospecting alone accounts for 6 of the 159, against 4 official servers in that category, which is the one place in this data where the community outbuilt the vendors.

## What to check before depending on a community server

- When the repo last moved. This directory does not publish that yet: the repo health rail has not been run, and a star count without the date it was taken is a lie, so nothing is shown rather than something stale. 218 entries already carry a github.com URL somewhere in their fields, which is the seed for that work.
...
The 159 community servers in this directory are listed with their URLs and their auth models, and every one links to the entry it came from.
