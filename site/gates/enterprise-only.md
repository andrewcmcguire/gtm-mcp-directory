...
> 79 of 649 GTM tools in this directory are enterprise only. API access needs a contract, a seat count, or a procurement cycle. A solo operator is out.
...
79 of 649 entries. Ordered by the published rule: official MCP first, then community, then unknown, then n/a, then none-found; within each band gate order is free, paid, enterprise-leaning, enterprise-only, unknown; then alphabetical by name. Computed, never curated, never purchasable.
