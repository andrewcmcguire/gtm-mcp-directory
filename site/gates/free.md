...
> 78 of 1,032 GTM tools in this directory are free to start. A solo operator can get API access without talking to anyone.
...
78 of 1,032 entries. Ordered by the published rule: official MCP first, then community, then unknown, then n/a, then none-found; within each band gate order is free, paid, enterprise-leaning, enterprise-only, unknown; then alphabetical by name. Computed, never curated, never purchasable.
