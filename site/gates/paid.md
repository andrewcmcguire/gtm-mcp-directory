...
> 132 of 784 GTM tools in this directory are paid, self-serve. A solo operator can get API access by paying, still without a sales call.
...
132 of 784 entries. Ordered by the published rule: official MCP first, then community, then unknown, then n/a, then none-found; within each band gate order is free, paid, enterprise-leaning, enterprise-only, unknown; then alphabetical by name. Computed, never curated, never purchasable.
