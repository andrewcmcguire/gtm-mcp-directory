# Apollo MCP by adelaidasofia: MCP server status, API access gate and what it does

> FastMCP server for Apollo.io - 22 tools: sequences, campaign health, mailbox warmup, enrichment, CRM,... Community MCP, Gate unknown. Checked 2026-09-12.
...
Vendor: [https://github.com/adelaidasofia/apollo-mcp](https://github.com/adelaidasofia/apollo-mcp) · entry id 01-apollo-mcp-by-adelaidasofia · source 01-data-enrichment.md line 4136
...
https://github.com/adelaidasofia/apollo-mcp

- [https://github.com/adelaidasofia/apollo-mcp](https://github.com/adelaidasofia/apollo-mcp)
...
- **Repo read**: adelaidasofia/apollo-mcp
...
- **APOLLO_API_KEY** Yes evidence: in a README table · calling it reads · from a third party repo, so these are that author's tools, not the vendor's published surface

- **APOLLO_AUDIT_LOG_PATH** No evidence: in a README table · calling it reads · from a third party repo, so these are that author's tools, not the vendor's published surface

- **APOLLO_MCP_CONFIG** No evidence: in a README table · calling it reads · from a third party repo, so these are that author's tools, not the vendor's published surface

- **APOLLO_MCP_TIMEOUT** No evidence: in a README table · calling it reads · from a third party repo, so these are that author's tools, not the vendor's published surface

- **Var** Required evidence: in a README table · calling it reads · from a third party repo, so these are that author's tools, not the vendor's published surface

- **daily_send_target** 50 evidence: in a README table · calling it reads · from a third party repo, so these are that author's tools, not the vendor's published surface

- **mailbox_labels** `{}` evidence: in a README table · calling it reads · from a third party repo, so these are that author's tools, not the vendor's published surface

- **monthly_credit_pool** 4000 evidence: in a README table · calling it reads · from a third party repo, so these are that author's tools, not the vendor's published surface

- **sequence_labels** `{}` evidence: in a README table · calling it reads · from a third party repo, so these are that author's tools, not the vendor's published surface
